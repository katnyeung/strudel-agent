...
$chord: chord("<Cm7 Fm7 AbM7 G7>").voicing().sound("sawtooth").slow(4).lpf(1200).gain(0.3)

// ✅ ALSO CORRECT — angle brackets step one per cycle, no .slow() needed
$chord: chord("<Cm7 Fm7 AbM7 G7>").voicing().sound("sawtooth").lpf(1200).gain(0.3)
...
### Chords
- chord("Dm7 G7 CM7 Am7").voicing() — auto-voiced chord progression.
- .voicing() spreads notes across octaves naturally.
- Use angle brackets to step one chord per cycle: chord("<Dm7 G7 CM7 Am7>")
- Roots: C, D, E, F, G, A, B and flats: Bb, Eb, Ab, Db, Gb. ⚠️ Avoid sharp roots (C#, D#, G#) — they often fail.
- ⚠️ CRITICAL: "maj7" is NOT valid. Use "M7" or "^7" for major 7th chords.
- Valid chord suffixes: m, M, 7, m7, M7, ^7, m9, M9, 9, 6, 69, add9, madd9, sus, aug, o (dim), h (half-dim), m7b5, 11, 13, m11, 7b9, 7#9, 7#11, 7b5, 7#5, 7sus, 9sus, 13sus, m6, m69, mM7, m^7
- Examples: Cm7, FM7, Bb^7, G7, Dm9, EbM7, Am7b5, G7#9
...
## Synths (built-in oscillators — always available)
...
## Sample-based instruments (from default sample map — always available)
Melodic: piano, casio, pluck, pad
Kits/Textures: jazz, east, metal, space, wind, insect, crow, numbers
- Use .sound("piano") for acoustic piano. Use .sound("casio") for lo-fi keyboard.
- ⚠️ Do NOT invent sound names like "ep", "rhodes", "keys", "electric_piano" — they don't exist.
- ⚠️ Only use: built-in synths (above), sample names (piano, casio, pluck, pad, jazz, etc.), or drum names (bd, sd, hh, etc.). If unsure, stick to synths — they always work.
